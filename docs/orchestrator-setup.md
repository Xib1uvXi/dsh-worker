...
Produce a complete dsh-orchestrator/SKILL.md and references/local-setup.md
...
Expected output is a personal directory with these two files:
...
You can also copy and customize the template manually. Preserve both files together, replace every `REPLACE_...` value in `references/local-setup.md`, and verify every chosen method path. The copied skill's relative link stays inside its own directory; obtain product documentation through the installed CLI instead of preserving relative links into the original checkout.
