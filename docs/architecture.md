...
Native session statistics are projected into durable `worker/stats` events at checkpoints/closed steps and replayed through the existing trajectory API. This observation never changes ticket acceptance. Ralph is explicitly disabled alongside subagent/workflow tools. See [Worker capabilities](plugins.md).
