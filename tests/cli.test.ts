...
function invoke(home: string, args: string[], input?: unknown) {
  return new Promise<{ code: number | null; value: any; stderr: string }>(
    (resolve, reject) => {
      const child = spawn(
        process.execPath,
      ...
      child.on("close", (code) => {
        try {
          resolve({
            code,
            value: stdout ? JSON.parse(stdout) : undefined,
            stderr,
          });
        } catch (e) {
          reject(e);
...
async function setup() {
  ...
  return {
    ...
    c,
    http,
    cli: (args: string[], input?: unknown) => invoke(f.home, args, input),
    close: async () => {
      await http.close();
      await c.close();
    },
...
it("diagnoses failed attempts without mutations and retains history after successful recovery", async () => {
  ...
  try {
    ...
    const status = await s.cli(["status", s.ticket.ticketId, "--summary"]);
    expect(status.value.state).toBe("interrupted");
    expect(status.value.ticket).toBeUndefined();
    expect(status.value.attempt.cleanExit).toBe(true);
    const report = await s.cli(["diagnose", s.ticket.ticketId]);
    expect(report.code).toBe(1);
    expect(report.value.nextSteps[0].argv).toContain("recover");
    const issues = await s.cli([
      "errors",
      s.ticket.ticketId,
      "--attempt",
    ...
    expect(
      issues.value.issues.some((e: any) => e.message.includes("Delivery")),
    ).toBe(true);
    ...
    const current = await s.cli(["diagnose", s.ticket.ticketId]);
    ...
    const list = await s.cli(["list", "--summary"]);
    expect(list.value.tickets[0].ticketId).toBe(s.ticket.ticketId);
    expect(list.value.tickets[0].attempts).toBeUndefined();
  } finally {
    await s.close();
  }
}, 20000);
it("reports actual verification exit/output with explicit truncation and full stored output", async () => {
  ...
  try {
    ...
    s.c.run(s.ticket.ticketId);
    await s.c.wait(s.ticket.ticketId);
    await s.c.verify(s.ticket.ticketId);
    await s.c.wait(s.ticket.ticketId);
    const short = await s.cli(["errors", s.ticket.ticketId]);
    const e = short.value.issues.find((e: any) => e.exitCode === 7);
    expect(e.output.length).toBe(4000);
    expect(e.output).toContain("VERIFICATION_FAILED");
    expect(e.outputTruncated).toBe(true);
    const full = await s.cli(["errors", s.ticket.ticketId, "--full"]);
    const f = full.value.issues.find((e: any) => e.exitCode === 7);
    expect(f.output.length).toBeGreaterThan(5000);
    expect(f.outputTruncated).toBe(false);
    expect((await s.cli(["diagnose", s.ticket.ticketId])).code).toBe(1);
  } finally {
    await s.close();
  }
}, 15000);
it("health distinguishes missing discovery and authentication failure without exposing tokens", async () => {
  ...
  const missing = await invoke(join(fixture().root, "not-started"), ["health"]);
  expect(missing.code).toBe(1);
  expect(
    missing.value.checks.some((c: any) => c.code === "service_missing"),
  ).toBe(true);
  const s = await setup();
  try {
    const healthy = await s.cli(["health"]);
    ...
    const auth = await s.cli(["health"]);
    expect(auth.code).toBe(1);
    expect(auth.value.checks.some((c: any) => c.code === "unauthorized")).toBe(
      true,
    );
    ...
    const corrupt = await s.cli(["health"]);
    expect(
      corrupt.value.checks.some((c: any) => c.code === "service_config"),
    ).toBe(true);
  } finally {
    await s.close();
  }
...
it.each([false, true])(
  ...
  async (prior) => {
    ...
    try {
      ...
      const history = await s.cli(["errors", s.ticket.ticketId]);
      expect(
...
it("CLI wait timeout leaves execution running; cancel and explicit recovery preserve control", async () => {
  ...
  try {
    ...
    const inspected = await s.cli(["recover", s.ticket.ticketId]);
    const continuation = {
      ticketId: s.ticket.ticketId,
      revision: 1,
