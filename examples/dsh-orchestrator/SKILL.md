...
Track each ticket's revision, attempt, worktree, dependencies and evidence in the selected task record. Use bounded waits and inspect status, brief, trajectory, activity or errors as needed. A wait timeout does not cancel execution or automatically wake this host. Use revision-bound instructions with stable IDs for scope-preserving guidance; running instructions wait for the next conversation turn. When cancelling because feedback is still unconsumed, use the bundled revision/attempt/instruction guard and reassess any rejection; inspect uncertain receipts before deciding on a resend. Recovery first accounts for old writers and does not itself execute a new attempt. Follow the current bundled recovery contracts for revisions and base changes.
...
## Two-level review when selected

When the task selects independent ticket reviewers plus final host review, follow [Two-level review](../../docs/two-level-review.md). Configure a batch pool tied to implementation concurrency, bind ticket revisions to `worker_then_astra`, and submit stable review requests. The service scales authorized reviewers within shared capacity; do not poll to manually open and close each process. A normal passing ticket needs host evidence adoption, not a default second full host ticket review. Keep external manual review for ordinary tickets.

The host still owns adjudication and acceptance. After combining all accepted deliveries, inspect the complete baseline-to-candidate diff, dependencies, non-ignored files and integration repairs, run complete candidate checks, and record separate final Spec and Standards findings before main. If the host authors substantive code, an independent reviewer must cover the complete final B-to-C candidate. Recheck main and candidate identities before integration; drift invalidates the old final judgment. Never downgrade a two-level ticket to a bare acceptance when the service lacks review support.
